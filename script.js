...
function normalizeData(rows) {
  return rows.map(row => {
    ...
    // CORREÇÃO: Busca exata pelo cabeçalho da planilha
...
function populateMultiSelectOptions() {
  ...
  // CORREÇÃO: Geração dinâmica de meses sem restrição
  const mesesSet = {};
  allData.forEach(r => {
...
function toggleMultiSelect(id) {
  ...
  state.open = !state.open;
  if (state.open) { wrapper.classList.add('open'); const si = wrapper.querySelector('.multi-select-search input'); if (si) si.value = ''; renderMultiSelect(id); }
...
function selectAllMultiSelect(id) {
  const state = multiSelectState[id]; if (!state) return;
  state.selected = [...state.values]; renderMultiSelect(id); applyFilters();
}
function clearMultiSelect(id) {
  const state = multiSelectState[id]; if (!state) return;
  state.selected = []; renderMultiSelect(id); applyFilters();
}
function filterMultiSelectOptions(id, query) {
  const state = multiSelectState[id]; if (!state) return;
  if (!query) { state.values = [...state._allValues]; }
  else { const q = query.toLowerCase(); state.values = state._allValues.filter(v => v.toLowerCase().includes(q)); }
  renderMultiSelect(id);
}
function getMultiSelectValues(id) {
  const state = multiSelectState[id]; return state ? state.selected : [];
}
document.addEventListener('click', function(e) {
  if (!e.target.closest('.multi-select-wrapper')) {
    Object.keys(multiSelectState).forEach(key => {
      multiSelectState[key].open = false; const w = document.getElementById(key); if (w) w.classList.remove('open');
    });
  }
});
...
async function fetchFreshData() {
  ...
  return new Promise((resolve, reject) => {
    Papa.parse(text, {
      ...
      error(err) { console.error('Parse error:', err); reject(err); }
    });
  });
}
async function fetchDataInBackground() {
...
// ============================================================
function setCachedData(data, timestamp) {
  try { localStorage.setItem(CACHE_KEY, JSON.stringify({ data, timestamp: timestamp || new Date().toISOString() })); }
...
function getCachedData(ignoreExpiry = false) {
  try {
    ...
    const cacheDate = new Date(cacheObject.timestamp);
    const hoursDiff = (now - cacheDate) / (1000 * 60 * 60);
    return hoursDiff <= CACHE_EXPIRY_HOURS ? cacheObject : null;
  } catch (e) { return null; }
}
function onDataLoaded() { populateMultiSelectOptions(); applyFilters(); }

// ============================================================
// NORMALIZAR DADOS (função normalizeData completa)
// ============================================================
function normalizeData(rows) {
  return rows.map(row => {
    const get = (...keys) => {
    ...
    const unidadeSolicitante = getUnidade({ 'UNIDADE SOLICITANTE': get('UNIDADE SOLICITANTE'), 'UNIDADE DE REFERÊNCIA': get('UNIDADE DE REFERÊNCIA','UNIDADE DE REFERENCIA') });
    const nomeCBO = get('NOME CBO');
    const especialidade = get('ESPECIALIDADE');
    const cbof = formatCBO(nomeCBO, especialidade);
    const profissional = formatProfissional(get('NOME PROFISSIONAL'));
    ...
    return {
      _raw: row, unidadeExecutante: unidadeExec, unidadeSolicitante, cbo: cbof,
      especialidade: especialidade ? titleCase(especialidade.toString()) : '', nomeCBO,
      profissional, tipoAtendimento: tipoAtend, situacao, situacaoLabel: getSituacaoLabel(situacao),
      operadorCod: operCod, operador: getOperador(operCod), dataCriacao, dataCriacaoParsed,
      dataAgenda, dataAgendaParsed, mesAgendamento: mesLabel, distrito,
      valor: parseFloat((get('VALOR')||'0').toString().replace(',','.')) || 0,
      quantidade: parseInt(get('QUANTIDADE')||'1') || 1,
    };
  }).filter(r => r.unidadeExecutante || r.profissional);
...
function applyFilters() {
  ...
  const tabelaPanel = document.getElementById('tab-tabela');
  if (tabelaPanel && tabelaPanel.classList.contains('active')) { renderTable(); }
}
function clearFilters() {
  Object.keys(multiSelectState).forEach(key => {
    multiSelectState[key].selected = [];
